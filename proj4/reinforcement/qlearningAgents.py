...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in actions:
            ...
            elif self.getQValue(state, action) == Qvalue:
                optimal_action_list.append(action)
        return random.choice(optimal_action_list)
        # util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        newValue += self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        QValue = 0.0
        i = 0
        # k = self.featExtractor.getFeatures(state, action)
        for feature in self.featExtractor.getFeatures(state, action):
            # p = self.getWeights()
            QValue += self.featExtractor.getFeatures(state, action)[feature] * self.weights[feature]
            i += 1
        return QValue
