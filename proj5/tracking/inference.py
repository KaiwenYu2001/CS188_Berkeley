    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            ...
            if len(factor.unconditionedVariables()) == 1:
                continue
        #     # print(factor)
        #     newfactor = eliminate(factor, elimvar)
        ...
        # return normalize(joinFactors(currentfactorslist))
            ...
            # Append the factor back to the factors list
            # print(factorsList)
            factorsList.append(factor)
            # print(factorsList)

        # Join all the factors together and normalize the final factor
        finalFactor = normalize(joinFactors(factorsList))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            self.beliefs[position] = self.getObservationProb(observation, pacPosition, position, jailPosition) * \
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_pos in self.allPositions:
            transition_probs[old_pos] = self.getPositionDistribution(gameState, old_pos)

        # Update the beliefs based on the transition probabilities
        new_beliefs = util.Counter()
        for new_pos in self.allPositions:
            for old_pos in transition_probs.keys():
                new_beliefs[new_pos] += transition_probs[old_pos][new_pos] * self.beliefs[old_pos]

        # Update the beliefs of the ghost agent with the new beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            probability = self.getObservationProb(observation, gameState.getPacmanPosition(), particle,
                                                  self.getJailPosition())
            if weights.get(particle) == 0:
                weights[particle] = probability
            else:
                weights[particle] += probability
        if weights.total() != 0:
            weights.normalize()
            self.particles = [weights.sample() for _ in self.particles]
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        newparticles = []
        for particle in self.particles:
            if not dists.get(particle):
                dists[particle] = self.getPositionDistribution(gameState, particle)
            tmp = dists.get(particle)
            newparticles.append(tmp.sample())
