...
def sentence1() -> Expr:
    ...
    # correct result: "((A | B) & (~A <=> (~B | C)) & (~A | ~B | C))"
    expression1 = Expr('A') | Expr('B')
    expression2 = Expr('<=>', Expr('~', Expr('A')), Expr('|', Expr('~', Expr('B')), Expr('C')))
    expression3 = Expr('|', Expr('~', Expr('A')), Expr('~', Expr('B')), Expr('C'))
    result = Expr('&', expression1, expression2, expression3)
    return result
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # correctresult: "((C <=> (B | D)) & (A >> (~B & ~D)) & (~(B & ~C) >> A) & (~D >> C))"
    A = Expr('A')
    B = Expr('B')
    C = Expr('C')
    D = Expr('D')
    expression1 = Expr('<=>', C, Expr('|', B, D))
    expression1 = conjoin(C % (B | D))
    expression2 = Expr('>>', A, Expr('&', Expr('~', B), Expr('~', D)))
    expression3 = Expr('>>', Expr('~', (B & Expr('~', C))), A)
    expression4 = Expr('>>', Expr('~', D), C)
    result = Expr('&', expression1, expression2, expression3, expression4)
    return result
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # correct result: "((PacmanAlive_1 <=> ((PacmanAlive_0 & ~PacmanKilled_0) | " \
    #                 "(~PacmanAlive_0 & PacmanBorn_0))) & " \
    #                 "~(PacmanAlive_0 & PacmanBorn_0) & " \
    #                 "PacmanBorn_0)"
    # R = conjoin(Expr('A') & Expr('B') & Expr('C') & Expr('D'))
    # L = conjoin(Expr('A'), Expr('B'), Expr('C'), Expr('D'))
    con1 = conjoin(PropSymbolExpr('PacmanAlive', time=1))
    con2 = conjoin((PropSymbolExpr('PacmanAlive', time=0) & ~ PropSymbolExpr('PacmanKilled', time=0)) | (
            ~ PropSymbolExpr('PacmanAlive', time=0) & PropSymbolExpr('PacmanBorn', time=0)))
    con3 = conjoin(~ (PropSymbolExpr('PacmanAlive', time=0) & PropSymbolExpr('PacmanBorn', time=0)))
    con4 = PropSymbolExpr('PacmanBorn', time=0)
    result = conjoin((con1 % con2), con3, con4)
    return result
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # l = findModel(premise & (~ conclusion))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    result = conjoin(atLeastOne(literals), atMostOne(literals))
    return result
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]] = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    result = PropSymbolExpr(pacman_str, x, y, time=now) % disjoin(*possible_causes)
    return result
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    for x, y in non_outer_wall_coords:
        expressions.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(expressions))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    ismodel = findModel(conjoin(*KB))
    KB[6] = ~KB[6]
    isnotmodel = findModel(conjoin(*KB))
    result = [ismodel, isnotmodel]
    print(result)
    return tuple(result)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        if findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(*KB)):
            return extractActionSequence(findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(*KB)), actions)
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    def food_successor_axiom(x: int, y: int, time: int):
        now, next = time, time + 1
        rule1 = conjoin(PropSymbolExpr(food_str, x, y, time=now), PropSymbolExpr(pacman_str, x, y, time=now)) >> \
               (~ PropSymbolExpr(food_str, x, y, time=next))
        rule2 = conjoin(PropSymbolExpr(food_str, x, y, time=now), (~ PropSymbolExpr(pacman_str, x, y, time=now))) >> \
                PropSymbolExpr(food_str, x, y, time=next)
        return conjoin(rule1, rule2)
    ...
    for t in range(50):
        print('timestep: ', t)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]))
        condition = [~ PropSymbolExpr(food_str, xf, yf, time=t) for (xf, yf) in food]
        if findModel(conjoin(*condition) & conjoin(*KB)):
            return extractActionSequence(findModel(conjoin(*condition) & conjoin(*KB)), actions)
        # if findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(*KB)):
        #     return extractActionSequence(findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(*KB)), actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))
        KB.extend([pacmanSuccessorAxiomSingle(x, y, t + 1, walls) for (x, y) in non_wall_coords])
        KB.extend([food_successor_axiom(x, y, t) for (x, y) in food])
    return []
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    #get initial location of pacman, add to KB
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))
    # KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    #update known_map accorfingly and add the appropriate expression to KB
